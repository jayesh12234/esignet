...
- OTP: under `mock`, `BasePage.getOtp()` always returns `111111`. Under `mosipid`, it polls mock SMTP (`smtpURL`) for the identity email (or phone). Call `markOtpRequestStart()` before requesting OTP so older messages are ignored. Mock MDS generates `device-dsk-partner.p12` at runtime from `p12EnvEndpoint` (do not commit a keystore).
