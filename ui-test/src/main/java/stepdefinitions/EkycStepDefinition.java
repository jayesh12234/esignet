...
public class EkycStepDefinition {
	...
	// Under the mock plugin (this environment, see config.properties: pluginToExecute) the login
	// completes directly to the relying party with no separate eKYC provider/terms/camera-preview
	// sequence at all - confirmed live via repeated screenshots. Rather than skipping the WHOLE
	// scenario the moment one such screen doesn't show up (which throws away every other step in
	// the scenario, including ones that don't depend on this screen), each individual method below
	// checks this and no-ops just its own single check, logging why, then lets the scenario carry on
	// to whatever steps follow. Only actually gates behavior on a real, verified environment fact
	// (the plugin), not a guess.
