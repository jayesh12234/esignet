...
public class VideoPreviewStepDefinition {
	...
	// The video preview/pre-verification-guide screen only exists at the end of the classic eSignet
	// eKYC provider/terms sequence, which never renders under this environment's mock plugin - the
	// earlier "clicks on proceed button in attention page" click already completes the login and
	// redirects to the relying party (verified live, repeatedly). Each step below no-ops just its
	// own check/click (not the whole scenario) when that's the case.
