...
Feature: Esignet Video Preview Page
  ...
  Scenario: Verifying PreVerification Guide And Video Preview Page
   ...
   # Pass 2: network dropped immediately after Proceed -> eKYC failure popup, then Okay returns to login
   ...
   # Pass 3: full successful run - slot polling, loading screen, redirect to Video eKYC, self-view, then a mid-liveness network drop
   And clicks on sign in with esignet button in login page
   When click on Language selection option
   And select the mandatory language
   And user click on Login with Otp
  ...
  # TC_Video_eKYC_Status_Screens_34 - Verify the error message, when KYC
  # verification fails and KYC provider do not allow retrials.
  #
  # This suite has no way to control an eKYC provider's retry-eligibility
  # configuration directly, so it can't trigger that exact business condition.
  # It reuses the one confirmed trigger already in this feature file for
  # reaching a terminal eKYC failure popup - a network drop immediately after
  # clicking Proceed on the camera preview page (see "Pass 2" in the first
  # scenario above) - and asserts the popup content this TC specifies.
  @smoke @VideoEkycStatusScreens
